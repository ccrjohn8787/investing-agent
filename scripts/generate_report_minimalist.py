#!/usr/bin/env python3
"""Generate minimalist HTML investment report for a ticker."""
...
def main():
    parser = argparse.ArgumentParser(description="Generate minimalist investment report")
    ...
    parser.add_argument(
        "--premium",
        action="store_true",
        help="Use premium LLM mode for narratives"
    )
    parser.add_argument(
        "--evaluate",
        action="store_true",
    ...
    parser.add_argument(
        "--disable-evidence",
        action="store_true",
        help="Disable evidence pipeline"
    )
    ...
    logger.info(f"Generating minimalist report for {ticker}")
    ...
    try:
        ...
        if HAS_LLM_WRITER and not args.disable_evidence:
            logger.info("Generating narratives with LLM...")
            try:
                # Set the appropriate model
                import os
                if args.premium:
                    os.environ['LLM_MODEL_OVERRIDE'] = 'gpt-4'
                    logger.info("  Using premium model: GPT-4")
                else:
                    os.environ['LLM_MODEL_OVERRIDE'] = 'gpt-4o-mini'
                    logger.info("  Using standard model: GPT-4o-mini")
            ...
            logger.info("Skipping LLM narrative generation (disabled or not available)")
            narratives = {
        ...
        if args.evaluate:
            ...
            try:
                from investing_agent.evals.report_quality.evaluator import ReportEvaluator
        ...
        # Save report
        report_path = output_dir / f"{ticker}_minimalist_report.html"
        ...
        logger.info(f"\n✅ Minimalist report generated successfully!")
        logger.info(f"   Report: {saved_path}")
        logger.info(f"   Data: {data_path}")
        
        # Print summary
        print(f"\n{'='*60}")
        print(f"MINIMALIST INVESTMENT REPORT - {ticker}")
        print(f"{'='*60}")
